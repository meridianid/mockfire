...
class JsonServer
{
    ...
    {
        $this->jsonDb = new DataBase(config('jsonserver.pathToDb'));
        // $this->jsonDb = new DataBase(public_path() . '' . Config::set('pathToDb', storage_path($db .'.json')));
    ...
    public function GET($tabName, $id, $parent, $data)
    {
        try {
            ...
            return call_user_func(config('jsonserver.resourceNotFound'), $this->response);
        }
    ...
    {
        if (!(config('jsonserver.urlNamingForm') === config('jsonserver.tableNamingForm'))) {
            $method = config('jsonserver.tableNamingForm');
